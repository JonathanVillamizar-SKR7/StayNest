...
<main style="background-color: var(--secondary-color);">
	...
	<nav class="navbar navbar-expand-lg py-3" style="background-color: var(--secondary-color);">
		<div class="container d-flex justify-content-between align-items-center">
			...
			<form class="form-inline d-flex ms-auto">
				...
				<button class="btn Search my-2 my-sm-0" style="color: var(--warning-color); background-color: var(--primary-color);" type="submit">Search</button>
			</form>
		</div>
	</nav>
		...
		<div class="card-deck p-5">
			<div class="row">
				...
				for (Housing f : house) {
					...
					String firstImg = (imgs != null && !imgs.isEmpty()) ? request.getContextPath() + imgs.get(0).getImgRoute() : "img/default.jpg";
				%>
				<div class="col-md-4 mb-3">
					<a href="housing.jsp?id=<%=f.getIdHouse()%>" class="text-decoration-none">
						<div class="card" style="background-color: white; border-color: transparent;">
							<img class="card-img-top img-fluid" style="height: 25rem;" src="<%=firstImg%>" alt="Card image cap">
							<div class="card-body">
								<h3 class="card-title"><%=f.getName()%></h3>
								<h5 class="card-text text-muted"><%=f.getLocation()%></h5>
								<br>
								<h5 class="card-text text-muted"><%=f.getNumGuest()%> guests - <%=f.getNumBedroom()%> bedrooms - <%=f.getNumBed()%> beds - <%=f.getNumBath()%> bath</h5>
								<br>
								<h5 class="card-text"><%=f.getPrice()%> €</h5>
